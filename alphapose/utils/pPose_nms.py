...
def write_json(all_results,number, outputpath, form=None, for_eval=False):
	'''
	...
	if form == 'cmu': # the form of CMU-Pose
		...
		with open(os.path.join(outputpath,'alphapose-results.json'), 'r+') as json_file:
			json_file.truncate()
		...
		for i in range(0,raw_data_length):
			...
			elif data['Actions'][i] == "drinking":
				data['Actions'][i] = 1
					
			elif data['Actions'][i] == "jumping":
				data['Actions'][i] = 2

			elif data['Actions'][i] == "reading":
				data['Actions'][i] = 3

			elif data['Actions'][i] == "running":
				data['Actions'][i] = 4

			elif data['Actions'][i] == "phoning":
				data['Actions'][i] = 5
		...
		# Do prediction
		x=data1.drop(labels='Actions', axis=1)
		y=data1['Actions']
		results = []
		raw_data_length=data1.index
		raw_data_length=list(raw_data_length)
		raw_data_length=raw_data_length[-1]
		raw_data_length=raw_data_length+1
		print("data lenth each time :- ",raw_data_length)
		
		# checking the image number for onle 0th locn for this pass 0 ,otherwise do calculation
		n=number-1
		if n == 0:
			demo = x.iloc[0]
			demo = list(demo)
		...
		with open(os.path.join(outputpath,'final_xgboost_activity_prediction_model_67_50.pickle'), 'rb') as xgmodel_file:
			loaded_model = pickle.load(xgmodel_file) 
		
		with open(os.path.join(outputpath,'final_xgboost_activity_scaler_model.pickle'), 'rb') as scaler_file:
			scaler = pickle.load(scaler_file)
		
		prediction=loaded_model.predict(scaler.transform([demo]))
		print("model predictio is :- ",prediction)
		results.append(prediction)

		if results[0][0] == 0:
			output_prediction = "Applauding"
			print("result Applauding")
		elif results[0][0] == 1:
			output_prediction = "Drinking"
			print("result Drinking") 
		elif results[0][0] == 2:
			output_prediction = "Jumping"
			print("result Jumping")
		elif results[0][0] == 3:
			output_prediction = "Reading"
			print("result Reading")
		elif results[0][0] == 4:
			output_prediction = "Running"
			print("result Running")
		elif results[0][0] == 5:
			output_prediction = "Phoning"
			print("result Phoning")  
