	...
	def update(self):
		...
		while True:
			...
			if orig_img is None:
				...
				if self.save_video:
					stream.release()
				op_result=write_json(final_result, self.opt.outputpath, form=self.opt.format, for_eval=self.opt.eval)
			...
			if boxes is None or len(boxes) == 0:
				...
				if self.opt.save_img or self.save_video or self.opt.vis:
					...
					op_result=write_json(final_result,number, self.opt.outputpath, form=self.opt.format, for_eval=self.opt.eval)
					output_prediction=op_result
					print("after img second calling write json : result is ",output_prediction)
					self.write_image(img,output_prediction, im_name, stream=stream if self.save_video else None)

	def write_image(self, img,output_prediction, im_name, stream=None):
		if self.opt.vis:
			cv2.imshow("AlphaPose Demo", img)
			cv2.waitKey(30)
		if self.opt.save_img:
			print("prediction is ",output_prediction)                        
			img1=cv2.putText(img,output_prediction,(30,30),cv2.FONT_HERSHEY_SIMPLEX,1,(0, 0, 255),2,cv2.LINE_AA)
			print("putting text sucess")
			cv2.imwrite(os.path.join(self.opt.outputpath, 'vis', im_name), img1)
		if self.save_video:
			img1=cv2.putText(img,output_prediction,(30,30),cv2.FONT_HERSHEY_SIMPLEX,1,(0, 0, 255),2,cv2.LINE_AA)
			stream.write(img1)
